"""
Simple Reservoir Computer, Small-World topology + SANA-FE
===========================================================
A sine wave goes in. The reservoir (Watts-Strogatz small-world
network) processes it in Python. SANA-FE simulates the same
...
Reservoir topology: ring lattice with K=4 neighbours per side,
10% of edges randomly rewired to distant neurons (Watts-Strogatz).
...
K_NEIGHBORS  = 4       # each neuron connects to K nearest on each side
REWIRE       = 0.1     # fraction of local edges rewired to random targets
...
# ── Step 3: Reservoir, Small-World topology ───────────────────
# Neurons arranged in a ring, each connected to K nearest neighbours.
# A fraction REWIRE of those edges are randomly rewired to distant
# neurons (Watts-Strogatz). Gives short path lengths + high clustering.
rng = np.random.RandomState(SEED)

def make_small_world(n, k, rewire_prob, rng):
    W = np.zeros((n, n))
    for i in range(n):
    ...
    for i in range(n):
        for j in range(1, k + 1):
            if rng.rand() < rewire_prob:
                candidates = [t for t in range(n) if t != i and W[i, t] == 0]
                if candidates:
                    old_target = (i + j) % n
                    new_target = candidates[rng.randint(len(candidates))]
                    W[i, new_target] = W[i, old_target]
                    W[i, old_target] = 0.0
    return W

W = make_small_world(N_NEURONS, K_NEIGHBORS, REWIRE, rng)
...
ax.set_title("Signal vs Ridge Prediction, Membrane Voltage (Small-World)")
...
ax.legend(fontsize=10); ax.grid(alpha=0.2)
ax.set_xlabel("Time (s)"); ax.set_ylabel("Amplitude")
ax.set_title("Signal vs MLP Prediction, Membrane Voltage (Small-World)")
...
ax.legend(fontsize=10); ax.grid(alpha=0.2)
ax.set_xlabel("Time (s)"); ax.set_ylabel("Amplitude")
ax.set_title("Signal vs Ridge Prediction, Firing Rate (Small-World)")
...
ax.legend(fontsize=10); ax.grid(alpha=0.2)
ax.set_xlabel("Time (s)"); ax.set_ylabel("Amplitude")
ax.set_title("Signal vs MLP Prediction, Firing Rate (Small-World)")
...
ax.set_title("Ridge Accuracy, Membrane Voltage (Small-World)")
...
ax.legend(); ax.grid(alpha=0.3)
ax.set_xlabel("Time (s)"); ax.set_ylabel("Absolute Error")
ax.set_title("MLP Accuracy, Membrane Voltage (Small-World)")
...
ax.legend(); ax.grid(alpha=0.3)
ax.set_xlabel("Time (s)"); ax.set_ylabel("Absolute Error")
ax.set_title("Ridge Accuracy, Firing Rate (Small-World)")
...
ax.legend(); ax.grid(alpha=0.3)
ax.set_xlabel("Time (s)"); ax.set_ylabel("Absolute Error")
ax.set_title("MLP Accuracy, Firing Rate (Small-World)")
...
ax.set_title("SANA-FE Chip Energy per Sample Window, Small-World (Inference)")
...
ax.set_title("Energy Comparison: Chip vs CPU, Small-World (Full Run)")
